...
export const endpoints = {
  ...
  projects: {
    ...
    create: () => "/projects", // Added for creating projects
  },
...
function handleApiError(error: any): void {
  if (axios.isAxiosError(error)) {
    if (error.response) {
      ...
      output.info("Please check your internet connection and the API URL.");
      output.info(`Current API URL: ${getApiUrl()}`);
      output.info(
        "You can change the API URL with 'todo config set-api-url <url>'"
      );
    } else {
...
/**
 * Refresh the API client's base URL
 * Call this when the API URL changes
 */
export function refreshApiConfig(): void {
  // Create a new instance with the updated base URL
